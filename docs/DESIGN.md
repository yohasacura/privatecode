...
|---|---|---|
| **form factor** | **standalone desktop app** (Tauri). Briefly reversed to a VS Code extension on 2026-08-03 and reversed back the same day: the extension surface was a chat participant wedged into Copilot's chat view, which is not the "full working chat window" this tool is for. That attempt (`ext/`) was carried unbuilt for a while and removed from the tree on 2026-08-24; it is in git history if anyone wants to read it. The UI was rebuilt instead — sessions rail \| chat \| context panel. | user requirement, tested against a built artefact twice |
...
| **UI** | three columns: sessions rail · chat · workspace panel. The panel has **four** tabs — Changes · History · Files · Terminal — and Changes leads | the conversation is the tool, the other two columns are consulted. Five tabs did not fit (398px of tabs in a 380px panel, the fifth 19px past the window edge, the whole shell scrolling sideways); Jobs and Terminal were one question asked twice, so running processes are pinned above the console instead of given a tab. Changes leads because this column answers "what has the agent done to my workspace" and a file tree answers a question you could ask before it started |
| **UI: one vocabulary** | one row component and one empty state for every list in the panel; a test that fails on a CSS token used but never declared, or a class a component asks for that the stylesheet does not define | a stylesheet has no compiler. Three features in a row were written against `--line`/`--danger`/`--warn`, which do not exist, in eleven places — and an unresolvable `var()` deletes the whole declaration, so History, the parked-decisions card, the MCP list and the rewind confirmation silently lost their borders and the one destructive button lost its red. Nothing failed; the panels just looked unfinished |
